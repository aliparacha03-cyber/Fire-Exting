...
export default async function ReportsPage() {
  const [{ data: inspections, error }, { count: totalUnits }] = await Promise.all([
  ...
  return (
    <AuthGuard>
      <Nav />
      <main style={{maxWidth:960,margin:'0 auto',padding:'36px 20px 60px'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:12,marginBottom:32}}>
          {[
            {label:'Total Inspections',value:total,  color:'var(--text)'},
            {label:'Passed',           value:passes, color:'var(--green)'},
            {label:'Failed',           value:fails,  color:'var(--red)'},
          ].map(s => (
